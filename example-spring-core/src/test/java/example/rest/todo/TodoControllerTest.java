...
import example.rest.AbstractContollerTest;
...
public class TodoControllerTest extends AbstractContollerTest {
	...
	public void shouldList() throws Exception {
		...
		// when & then
		perform(get("/todos").accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
	...
	public void shouldGet() throws Exception {
		...
		// when & then
		perform(get("/todos/{id}", id).accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk()).andExpect(
				jsonPath("$.title").value(equalTo(todo.getTitle())));
	}
	...
	public void shouldAdd() throws Exception {
		...
		// when & then
		perform(post("/todos").param("title", title).accept(MediaType.APPLICATION_JSON))
	...
	public void shouldDelete() throws Exception {
		...
		// when & then
		perform(delete("/todos/{id}", id).accept(MediaType.APPLICATION_JSON)).andExpect(status().isNoContent());
	...
	public void shouldUpdateStatusToDone() throws Exception {
		...
		// when & then
		perform(put("/todos/{id}/done", id).accept(MediaType.APPLICATION_JSON)).andExpect(status().isNoContent());
	...
	public void shouldUpdateTitle() throws Exception {
		...
		// when & then
		perform(put("/todos/{id}/title", id).param("title", title).accept(MediaType.APPLICATION_JSON)).andExpect(
				status().isNoContent());
