...
/**
 * Message converter that uses the Jackson library to convert messages to and from JSON. Maps an object to a
 * {@link BytesMessage}, or to a {@link TextMessage} if the {@link #setTargetType targetType} is set to
 * {@link MessageType#TEXT}. Converts from a {@link TextMessage} or {@link BytesMessage} to an object.
 * 
 * @author Mark Pollack
 * @author Dave Syer
...
public class MappingJackson2MessageConverter implements MessageConverter {
	...
	/**
	 * Specify whether {@link #toMessage(Object, Session)} should marshal to a {@link BytesMessage} or a
	 * {@link TextMessage}.
	 * <p>
	 * The default is {@link MessageType#BYTES}, i.e. this converter marshals to a {@link BytesMessage}. Note that the
	 * default version of this converter supports {@link MessageType#BYTES} and {@link MessageType#TEXT} only.
	 * 
	...
	/**
	 * Specify the encoding to use when converting to and from text-based message body content. The default encoding
	 * will be "UTF-8".
	 * <p>
	 * When reading from a a text-based message, an encoding may have been suggested through a special JMS property
	 * which will then be preferred over the encoding set on this MessageConverter instance.
	 * 
	...
	/**
	 * Specify the name of the JMS message property that carries the encoding from bytes to String and back is
	 * BytesMessage is used during the conversion process.
	 * <p>
	 * Default is none. Setting this property is optional; if not set, UTF-8 will be used for decoding any incoming
	 * bytes message.
	 * 
	 * @see #setEncoding
	 */
	...
	/**
	 * Specify the name of the JMS message property that carries the type id for the contained object: either a mapped
	 * id value or a raw Java class name.
	 * <p>
	 * Default is none. <b>NOTE: This property needs to be set in order to allow for converting from an incoming message
	 * to a Java object.</b>
	 * 
	...
	/**
	 * Specify mappings from type ids to Java classes, if desired. This allows for synthetic ids in the type id message
	 * property, instead of transferring Java class names.
	 * <p>
	 * Default is no custom mappings, i.e. transferring raw Java class names.
	 * 
	 * @param typeIdMappings
	...
	/**
	 * Template method that allows for custom message mapping. Invoked when {@link #setTargetType} is not
	 * {@link MessageType#TEXT} or {@link MessageType#BYTES}.
	 * <p>
	 ...
	 * <p>
	 * The default implementation consults the configured type id mapping and sets the resulting value (either a mapped
	 * id or the raw Java class name) into the configured type id message property.
	 * 
	 * @param object
	 *            the payload object to set a type id for
	 * @param message
	 ...
	 *             in case of I/O errors
	 */
	protected Object convertFromTextMessage(TextMessage message, JavaType targetJavaType) throws JMSException,
			IOException {
	 ...
	 *             in case of I/O errors
	 */
	protected Object convertFromBytesMessage(BytesMessage message, JavaType targetJavaType) throws JMSException,
			IOException {
	 ...
	 * <p>
	 * The default implementation parses the configured type id property name and consults the configured type id
	 * mapping. This can be overridden with a different strategy, e.g. doing some heuristics based on message origin.
	 * 
	 * @param message
	 *            the JMS Message to set the type id on
	 * @throws JMSException
	...
	protected JavaType getJavaTypeForMessage(Message message) throws JMSException {
		...
		try {
			return objectMapper.constructType(ClassUtils.forName(typeId, getClass().getClassLoader()));
		} catch (Throwable ex) {
